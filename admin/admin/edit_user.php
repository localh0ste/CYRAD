...
if (isset($_GET['username'])) {
    ...
    // Fetch from data_limits (may not exist)
    $stmt = $conn->prepare("SELECT internet_type, internet_limit FROM data_limits WHERE username = ?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    $result = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_user'])) {
    ...
    if (empty($new_username) || empty($internet_type) || ($internet_type === 'limited' && empty($internet_limit_mb))) {
        ...
        if (!empty($password)) {
            ...
            $stmt->bind_param("sss", $new_username, $hashedPassword, $original_username);
        } else {
        ...
        // Handle data_limits
        $stmt = $conn->prepare("SELECT username FROM data_limits WHERE username = ?");
        $stmt->bind_param("s", $original_username);
        $stmt->execute();
        $stmt->store_result();
        ...
        if ($internet_type === 'limited') {
            ...
            if ($exists) {
                $stmt = $conn->prepare("UPDATE data_limits SET username=?, internet_type=?, internet_limit=? WHERE username=?");
                $stmt->bind_param("ssis", $new_username, $internet_type, $limit_bytes, $original_username);
            } else {
                $stmt = $conn->prepare("INSERT INTO data_limits (username, internet_type, internet_limit) VALUES (?, ?, ?)");
                $stmt->bind_param("ssi", $new_username, $internet_type, $limit_bytes);
            }
            $stmt->execute();
            $stmt->close();
        } else {
